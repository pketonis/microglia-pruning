...
**Learning:** `torch.special.entr` is significantly faster and more numerically stable than manual `- (p * p.log())` entropy calculation. `torch.amax` is faster than `torch.max(...)[0]` when indices are not needed.
